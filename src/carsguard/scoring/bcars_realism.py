...
def score_bcars_realism(
    ...
    """
    Score how experimentally plausible a BCARS/CARS spectrum is relative
    to a real coherent-spectroscopy reference profile.
    """
    selected_features = selected_features or DEFAULT_BCARS_FEATURES
    ...
    for feature_name in selected_features:
        ...
        if score < 0.25:
            warnings.append(
                f"{feature_name} is far outside the real BCARS/CARS reference range"
            )
        elif score < 0.5:
            warnings.append(
                f"{feature_name} is near the edge of the real BCARS/CARS reference range"
            )
